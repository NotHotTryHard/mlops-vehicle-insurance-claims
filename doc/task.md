...
Документ предназначен для преподавателя/ревьюера: фиксирует **постановку**, **источники данных**, **контроль качества**, **валидацию**, **сохранение артефактов** и отдельно — **проектирование моделей** (гиперпараметры, признаковое пространство, выбор семейства). Все пути и ключи конфига приведены относительно корня репозитория; актуальная схема конфигурации — файл `config.yaml` (ниже значения соответствуют состоянию репозитория на момент написания).
