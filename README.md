...
├── analyse -> generate EDA report from SQLite data 
├── train
│   ├── данные: из CSV (--path-csv) или из БД (--date-until)
│   ├── --new <catboost|mlp> -> создать модель, обучить, сохранить
│   └── --old <model_name> -> загрузить модель, дообучить, сохранить новую версию
└── val
    ├── данные: из CSV (--path-csv) или из БД (--date-until)
...
Каждая версия модели сохраняется одним pickle-файлом с timestamp:
...
### Хранилище данных — `src/data` (SQLite)
